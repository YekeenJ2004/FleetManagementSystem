...
class Vehicle:
    """
    Represents a vehicle with various attributes such as type,
    registration number, tax status, fuel type, and manufacture year.

    Attributes:
        vehicle_type (str): The type of the vehicle (e.g., SUV, Sedan).
        reg_number (str): The registration number of the vehicle.
        tax_status (str): The tax status of the vehicle
            (e.g., Paid, Unpaid).
        tax_type (str): The type of tax applicable to the vehicle.
        tax_due_date (Optional[str]): The due date for tax payment
            (format: YYYY-MM-DD).
        ...
        for attr_name, (value, expected_type) in attributes.items():
            try:
                if not isinstance(value, expected_type):
                    raise TypeError(
                        f"{attr_name} must be of type {expected_type.__name__}"
                    )
